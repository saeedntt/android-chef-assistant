...
class UpdateShoppingListItemUseCase(private val shoppingListItemRepository: ShoppingListItemRepository) :
    ...
    override suspend fun execute(parameter: Pair<ShoppingList, ShoppingListItem>): Result<Unit> {
        return Result.success(
            shoppingListItemRepository.updateShoppingListItem(parameter.first, parameter.second)
        )
    }
}