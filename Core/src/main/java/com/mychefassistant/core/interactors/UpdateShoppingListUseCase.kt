...
class UpdateShoppingListUseCase(private val shoppingListRepository: ShoppingListRepository) :
    BaseSuspendUseCase<ShoppingList, Unit>() {
    override suspend fun execute(parameter: ShoppingList): Result<Unit> {
        return Result.success(shoppingListRepository.updateShoppingList(parameter))
    }
}